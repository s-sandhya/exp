...
def evaluateEER(user_scores, imposter_scores,auc_arr):
...
def evaluateEERGMM(user_scores, imposter_scores,auc_arr):
    ...
    for th in thresholds:
        ...
        FA = float(cnt2) / len(imposter_scores) 
        FR = float(cnt1) / len(user_scores)
        array[i, 0] = th
        array[i, 1] = FA
        array[i, 2] = FR
        i = i + 1
...
def manhattan_evaluate(subjects,auc_arr):
    ...
    for subject in subjects:        
        ...
        eers.append(evaluateEER(user_scores,imposter_scores,auc_arr))
...
def normed_eucledian_evaluate(subjects,auc_arr):
    ...
    for subject in subjects:        
        ...
        for i in range(test_genuine.shape[0]):
            ...
            user_scores.append(cur_score/(n1*n2))
      
        for i in range(test_imposter.shape[0]):
            cur_score = euclidean(test_imposter.iloc[i].values,mean_vector)
            n1=linalg.norm(test_imposter.iloc[i].values)
            n2=linalg.norm(mean_vector)
            imposter_scores.append(cur_score/(n1*n2))
...
def gmm_evaluate(subjects,auc_arr):
    ...
    for subject in subjects: 
        ...
        eers.append(evaluateEERGMM(user_scores, imposter_scores,auc_arr))
    return np.mean(eers),np.std(eers),np.mean(auc_arr)   

path ="D:\\fdrive\\sem8\\advanced analytics\\ca3\\DSL-StrongPasswordData.csv"
